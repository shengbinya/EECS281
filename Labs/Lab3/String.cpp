...
String& String::erase(size_t pos, size_t len) {
    ...
    char* insert = &cstr[pos];
    char* erase = insert + len;
    
    if (pos + len > sz || len == npos) {
        cstr[pos] = '\0';
        sz = pos;
    }
...
String& String::insert(size_t pos, const String& str_in) {
    // ERROR CHECKING: YOU DO NOT NEED TO WORRY ABOUT THIS FOR THIS LAB
...
size_t String::find_first_of(const String& str, size_t pos) {
    String str_used = *this;
    size_t end = npos;
    for (size_t i = 0; i < str.size(); i++) {
        size_t final = str_used.find(str.substr(i,1), pos);
        if (final < end)
            end = final;  
    }
    return end;
...
size_t String::find_last_of(const String& str, size_t pos) {
    ...
    if (pos < size())
        pos++;
    str_used = str_used.substr(0, pos);
    String operated = str_used;
    int i = 0;
    for (size_t j = str_used.size() - 1; j != npos; --j) {
        operated[i] = str_used[j];
        i++;
    }
    size_t index = operated.find_first_of(str);
    if (index == npos) {
        return npos;
    }
    else {
